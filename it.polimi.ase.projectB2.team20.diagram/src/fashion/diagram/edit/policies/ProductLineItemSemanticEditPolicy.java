...
public class ProductLineItemSemanticEditPolicy extends
	 ...
	 * @generated
	 */
	public ProductLineItemSemanticEditPolicy() {
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (FashionVisualIDRegistry.getVisualID(incomingLink) == BrandProductLinesEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(
						incomingLink.getSource().getElement(), null,
						incomingLink.getTarget().getElement(), false);
				cmd.add(new DestroyReferenceCommand(r));
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (FashionVisualIDRegistry.getVisualID(outgoingLink) == ProductLineBrandEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(
						outgoingLink.getSource().getElement(), null,
						outgoingLink.getTarget().getElement(), false);
				cmd.add(new DestroyReferenceCommand(r));
				...
				for (Iterator<?> cit = node.getChildren().iterator(); cit
					...
					case Product2EditPart.VISUAL_ID:
						for (Iterator<?> it = cnode.getTargetEdges().iterator(); it
							...
							if (FashionVisualIDRegistry
									.getVisualID(incomingLink) == ReviewProductEditPart.VISUAL_ID) {
								DestroyReferenceRequest r = new DestroyReferenceRequest(
										incomingLink.getSource().getElement(),
										null, incomingLink.getTarget()
												.getElement(), false);
						...
						for (Iterator<?> it = cnode.getSourceEdges().iterator(); it
							...
							if (FashionVisualIDRegistry
									.getVisualID(outgoingLink) == ProductReviewsEditPart.VISUAL_ID) {
								DestroyReferenceRequest r = new DestroyReferenceRequest(
										outgoingLink.getSource().getElement(),
										null, outgoingLink.getTarget()
												.getElement(), false);
